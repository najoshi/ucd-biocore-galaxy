...
def __main__():
    ...
    parser.add_option( '-J', '--junctions-output', dest='junctions_output_file', help='Junctions output file; format is BED.' )
    parser.add_option( '-H', '--hits-output', dest='accepted_hits_output_file', help='Accepted hits output file; format is BAM.' )
    parser.add_option( '-U', '--unmapped-output', dest='unmapped_output_file', help='Unmapped reads output file; format is BAM.' )
    parser.add_option( '-A', '--align-summary-file', dest='align_summary_file', help='File with summary of alignment.' )
    ...
    parser.add_option( '', '--transcriptome-only', action="store_true", dest='transcriptome_only', help="Map only to the transcriptome.")
    # Types of search.
    ...
    try:
        ...
        if options.unmapped_output_file:
            shutil.copyfile( os.path.join( "tophat_out", "unmapped.bam" ), options.unmapped_output_file )
        if options.align_summary_file:
            shutil.copyfile( os.path.join( "tophat_out", "align_summary.txt" ), options.align_summary_file )
