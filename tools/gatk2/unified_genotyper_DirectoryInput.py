...
def gatk_filename_from_galaxy( galaxy_filename, galaxy_ext, target_dir = None, prefix = None, picard_jar = None ):
...
##liubo added
    ...
    if options.datasets:
        for ( dataset_arg, filename, galaxy_ext, prefix ) in options.datasets:
            gatk_filename = gatk_filename_from_galaxy( filename, galaxy_ext, target_dir = tmp_dir, prefix = prefix , picard_jar = options.picard_jar)
            if dataset_arg:
	...
	inputBAM = ""
		...
		for name in files: 
		    if not name.startswith('.') and name.endswith('.bam'):
                        inputBAM += " -I " + os.path.join(root, name)
